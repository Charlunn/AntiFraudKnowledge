...
const { globalQuery, searchSuggestions } = storeToRefs(uiStore)
const { t } = useI18n()
...
<template>
  <aside class="relative flex h-full w-full flex-col overflow-hidden border-r border-border/60 bg-card/95 shadow-xl backdrop-blur">
          ...
          <div :key="activeTab" class="flex h-full flex-col gap-4">
            <template v-if="activeTab === 'filters'">
              <QuickFilters
                ...
                class="flex h-full flex-col rounded-2xl border border-border/50 bg-background/92 shadow-sm"
                @apply="applyFilters"
