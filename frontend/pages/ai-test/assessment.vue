  ...
  <div class="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 dark:from-dark-bg dark:to-dark-surface">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="max-w-4xl mx-auto">
        <!-- 开始评估 -->
        <div v-if="assessmentState === 'start'" class="bg-white dark:bg-dark-surface rounded-xl shadow-lg p-8 text-center">
          ...
          <h2 class="text-2xl font-semibold text-gray-900 dark:text-dark-text mb-4">准备开始AI智能评估</h2>
          <p class="text-gray-600 dark:text-dark-text-secondary mb-6 max-w-2xl mx-auto">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="text-center">
              ...
              <div class="text-sm text-gray-500 dark:text-dark-text-secondary">评估题目</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">5</div>
              <div class="text-sm text-gray-500 dark:text-dark-text-secondary">知识维度</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">15</div>
              <div class="text-sm text-gray-500 dark:text-dark-text-secondary">分钟时长</div>
            </div>
          </div>
          <button
        ...
        <!-- 评估进行中 -->
        <div v-else-if="assessmentState === 'progress'" class="bg-white dark:bg-dark-surface rounded-xl shadow-lg p-8">
          <!-- 进度条 -->
          <div class="mb-8">
            <div class="flex justify-between items-center mb-2">
              <span class="text-sm font-medium text-gray-700 dark:text-dark-text">评估进度</span>
              <span class="text-sm text-gray-500 dark:text-dark-text-secondary">{{ currentQuestion }}/{{ totalQuestions }}</span>
            </div>
          ...
          <!-- 当前题目 -->
          <div class="mb-8">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-dark-text mb-4">
            ...
            <div class="space-y-3">
              <label
                ...
                :key="index"
                class="flex items-center p-4 border border-gray-200 dark:border-dark-border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-bg transition-colors duration-200"
                :class="{ 'border-green-500 bg-green-50 dark:bg-green-900/20': selectedAnswer === index }"
              >
                <input
                  ...
                  class="sr-only"
                >
                <div class="w-4 h-4 border-2 border-gray-300 dark:border-dark-border rounded-full mr-3 flex items-center justify-center" :class="{ 'border-green-500': selectedAnswer === index }">
                  <div v-if="selectedAnswer === index" class="w-2 h-2 bg-green-500 rounded-full"></div>
                </div>
                <span class="text-gray-700 dark:text-dark-text">{{ option }}</span>
              </label>
            </div>
          </div>
          ...
          <div class="flex justify-between">
            <button
              ...
              class="px-6 py-2 border border-gray-300 dark:border-dark-border text-gray-700 dark:text-dark-text rounded-lg hover:bg-gray-50 dark:hover:bg-dark-bg disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
            >
              上一题
            </button>
            <button
        ...
        <!-- 评估结果 -->
        <div v-else-if="assessmentState === 'result'" class="bg-white dark:bg-dark-surface rounded-xl shadow-lg p-8">
          <div class="text-center mb-8">
            ...
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-dark-text mb-2">评估完成</h2>
            <p class="text-gray-600 dark:text-dark-text-secondary">您的反欺诈知识水平评估结果</p>
          </div>

          <!-- 总体得分 -->
          <div class="text-center mb-8">
            <div class="text-4xl font-bold text-green-600 dark:text-green-400 mb-2">{{ overallScore }}</div>
            <div class="text-lg text-gray-600 dark:text-dark-text-secondary">总体得分</div>
            <div class="text-sm text-gray-500 dark:text-dark-text-secondary mt-1">{{ getScoreLevel(overallScore) }}</div>
          </div>

          <!-- 各维度得分 -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div v-for="dimension in dimensionScores" :key="dimension.name" class="bg-gray-50 dark:bg-dark-bg rounded-lg p-4">
              <div class="flex justify-between items-center mb-2">
                <span class="font-medium text-gray-900 dark:text-dark-text">{{ dimension.name }}</span>
          ...
          <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-6 mb-6">
            <h3 class="text-lg font-semibold text-blue-900 dark:text-blue-300 mb-3">AI个性化建议</h3>
