  ...
  <div class="min-h-screen bg-gradient-to-br from-primary/10 to-primary/20 dark:from-dark-bg dark:to-dark-surface">
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
        <!-- 智能问答 -->
        <div class="bg-card dark:bg-card rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
          <div class="flex items-center mb-4">
            ...
            <h3 class="text-xl font-semibold text-foreground dark:text-foreground">智能问答</h3>
          </div>
          <p class="text-muted-foreground dark:text-muted-foreground mb-4">
            与AI助手对话，获取个性化的反欺诈知识解答和建议
          </p>
          <button 
            @click="startChat"
            class="w-full bg-primary/100 hover:bg-primary text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200"
          >
            开始对话
          </button>
        </div>
        ...
        <div class="bg-card dark:bg-card rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
          <div class="flex items-center mb-4">
            ...
            <h3 class="text-xl font-semibold text-foreground dark:text-foreground">智能评估</h3>
          </div>
          <p class="text-muted-foreground dark:text-muted-foreground mb-4">
          ...
          <button 
            ...
            class="w-full bg-primary/100 hover:bg-green-600 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200"
          >
            开始评估
          </button>
        </div>
        ...
        <div class="bg-card dark:bg-card rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300">
          <div class="flex items-center mb-4">
            ...
            <h3 class="text-xl font-semibold text-foreground dark:text-foreground">场景模拟</h3>
          </div>
          <p class="text-muted-foreground dark:text-muted-foreground mb-4">
          ...
          <button 
            ...
            class="w-full bg-primary/100 hover:bg-purple-600 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200"
          >
            开始模拟
          </button>
        </div>
      ...
      <div class="bg-card dark:bg-card rounded-xl shadow-lg p-6">
        <h2 class="text-2xl font-semibold text-foreground dark:text-foreground mb-6">最近的测试记录</h2>
        ...
        <div v-if="recentTests.length === 0" class="text-center py-8">
          ...
          <p class="text-muted-foreground dark:text-muted-foreground">暂无测试记录</p>
          <p class="text-sm text-muted-foreground dark:text-muted-foreground mt-2">开始您的第一次AI测试吧！</p>
        </div>
        ...
        <div v-else class="space-y-4">
          <div 
            ...
            :key="test.id"
            class="flex items-center justify-between p-4 border border-border dark:border-border rounded-lg hover:bg-muted/40 dark:hover:bg-muted/30 transition-colors duration-200"
          >
            <div class="flex items-center space-x-4">
              ...
              <div>
                <h3 class="font-medium text-foreground dark:text-foreground">{{ test.title }}</h3>
                <p class="text-sm text-muted-foreground dark:text-muted-foreground">{{ formatDate(test.createdAt) }}</p>
              </div>
            </div>
            <div class="flex items-center space-x-4">
              ...
              <button 
                ...
                class="text-primary hover:text-primary text-sm font-medium transition-colors duration-200"
              >
                查看详情
              </button>
            </div>
...
const getTestTypeClass = (type) => {
  const classes = {
    'chat': 'bg-primary/100',
    'assessment': 'bg-primary/100',
    'simulation': 'bg-primary/100'
  }
  return classes[type] || 'bg-muted/400'
}
...
const getScoreClass = (score) => {
  if (score >= 90) return 'text-primary dark:text-success-400'
