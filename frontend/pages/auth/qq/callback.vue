...
const handleQQCallback = async (code, state) => {
  try {
    ...
    const response = await $api('/api/users/oauth/qq/login/', {
      method: 'POST',
      body: {
        code,
        state
    ...
    if (response.success) {
      ...
      // 显示成功消息
      useNuxtApp().$toast?.success('QQ登录成功')
    ...
    error.value = err.data?.message || err.message || 'QQ登录失败'
  } finally {
    loading.value = false
  }
}
...
onMounted(async () => {
  ...
  await handleQQCallback(code, state)
})
</script>
