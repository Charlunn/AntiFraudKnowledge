...
class UserAchievement(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="achievements", verbose_name="用户"
    )
    achievement_type = models.CharField(
        max_length=50, verbose_name="成就类型"
    )  # 学习成就、模拟成就、分享成就等
...
class UserSkill(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="skills", verbose_name="用户"
    )
    skill_type = models.CharField(
        max_length=50, verbose_name="能力类型"
    )  # 信息识别能力、情绪应对能力、主动防御意识等
