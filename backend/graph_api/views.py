...
class FilteredGraphView(BaseGraphAPIView):
    ...
    def get(self, request, format=None):
        ...
        if not filter_prop or filter_value is None:
            ...
            logger.warning(
                "FilteredGraphView: Missing filter parameters. Returning initial graph as fallback."
            )
            # return Response({"error": "缺少过滤参数 'filter_prop' 和 'filter_value'"}, status=status.HTTP_400_BAD_REQUEST)
            # 或者，作为备选方案，返回初始图：
            try:
                results = db_utils.read_from_neo4j(
                    cypher_queries.GET_INITIAL_GRAPH_CYPHER
                )
                serializer = serializers.EchartsGraphSerializer(instance=results)
                return Response(serializer.data, status=status.HTTP_200_OK)
            except Exception as e:
                logger.exception(
                    "Error fetching initial graph data as fallback in FilteredGraphView."
                )
                raise e
        ...
        # 假设我们有一个查询模板字典
        query_template = (
            f"MATCH (n {{{filter_prop}: $value}})-[r]-(m) RETURN n, r, m LIMIT 50"
        )
        params = {"value": filter_value}
        query = query_template  # 在实际应用中，应从 cypher_queries.py 获取或构建
        logger.info(
            f"Fetching filtered graph data with query: {query} and params: {params}"
        )
        # --- 结束动态过滤示例 ---
...
class NodeDetailView(BaseGraphAPIView):
    ...
    def get(self, request, node_id, format=None):
        ...
        if not node_id:
            return Response(
                {"error": "缺少节点 ID"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            results = db_utils.read_from_neo4j(
                cypher_queries.GET_NODE_DETAIL_CYPHER, params=params
            )

            if not results:
                logger.warning(f"Node not found for node_id: {node_id}")
                return Response(
                    {"error": "未找到指定节点"}, status=status.HTTP_404_NOT_FOUND
                )

            # NodeDetailSerializer 期望接收记录列表
            serializer = serializers.NodeDetailSerializer(instance=results)
            logger.info(
                f"Node details for {node_id} fetched and serialized successfully."
            )
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception as e:
