...
def get_node_id(node_dict: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        logger.debug(
            f"Generated fallback hash ID {node_hash} for node dict: {node_dict}"
        )
...
class EchartsGraphSerializer(serializers.Serializer):
    ...
    def _format_node(self, node_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(node_dict, dict):
            logger.warning(
                f"Expected a dictionary for node, but got {type(node_dict)}. Skipping."
            )
            return None
        ...
        if node_id is None:
            logger.warning(
                f"Could not determine a unique ID for node: {node_dict}. Skipping node."
            )
            return None
    ...
    def _format_link(self, rel_tuple: Tuple) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(rel_tuple, tuple) or len(rel_tuple) != 3:
            logger.warning(
                f"Expected a tuple of length 3 for relationship, got {rel_tuple}. Skipping."
            )
            return None
        ...
        if not isinstance(start_node_dict, dict) or not isinstance(end_node_dict, dict):
            logger.warning(
                f"Relationship tuple contains non-dict nodes: {rel_tuple}. Skipping."
            )
            return None
        ...
        if source_id is None or target_id is None:
            logger.warning(
                f"Could not determine source/target ID for relationship: {rel_tuple}. Skipping link."
            )
            return None
        ...
        return {
            ...
            "label": {"show": True, "formatter": rel_type_str},
    ...
    def to_representation(
        self, instance: Union[List[Dict], Any]
        ...
        nodes_data_dict: Dict[str, Dict[str, Any]] = (
            {}
        )  # Stores unique nodes keyed by generated ID
        links_data: List[Dict[str, Any]] = []

        if not isinstance(instance, list):
            logger.warning(
                f"EchartsGraphSerializer received non-list input. Type: {type(instance)}. Returning empty graph."
            )
            return {"nodes": [], "links": []}

        if not instance:
            logger.info(
                "EchartsGraphSerializer received an empty list. Returning empty graph."
            )
            return {"nodes": [], "links": []}
        ...
        for i, record in enumerate(instance):
            if not isinstance(record, dict):
                logger.warning(
                    f"Record at index {i} is not a dict: {record}. Skipping."
                )
                continue
            ...
            if isinstance(rel_tuple, tuple) and len(rel_tuple) == 3:
                ...
                if start_node_id and end_node_id:
                    ...
                    logger.warning(
                        f"Skipping link due to missing node IDs in tuple: {rel_tuple}"
                    )

            elif (
                rel_tuple is not None
            ):  # Log if 'r' exists but isn't the expected tuple
                logger.warning(
                    f"Record {i} has 'r' key but it's not a valid relationship tuple: {rel_tuple}"
                )

        final_nodes_list = list(nodes_data_dict.values())
        logger.info(
            f"Serialized {len(final_nodes_list)} unique nodes and {len(links_data)} links for ECharts."
        )
...
class NodeDetailSerializer(serializers.Serializer):
    ...
    node_properties = serializers.DictField(
        read_only=True, help_text="Properties of the target node."
    )
    ...
    def to_representation(self, instance: Union[List[Dict], Any]) -> Dict[str, Any]:
        ...
        if not isinstance(instance, list):
            logger.warning(
                f"NodeDetailSerializer received non-list input. Type: {type(instance)}. Returning empty details."
            )
            return {"node_properties": {}, "neighbors": []}

        if not instance:
            logger.info(
                "NodeDetailSerializer received an empty list. Returning empty details."
            )
            return {"node_properties": {}, "neighbors": []}
        ...
        if isinstance(first_record, dict) and isinstance(first_record.get("n"), dict):
            ...
            if target_node_id:
                ...
                logger.info(
                    f"Processing details for target node identified as: {target_node_id}"
                )
            else:
                logger.error(
                    "NodeDetailSerializer: Could not determine ID for target node 'n' in first record."
                )
                return {"node_properties": {}, "neighbors": []}
        else:
            logger.error(
                "NodeDetailSerializer: Target node 'n' (dict) not found or invalid in the first record."
            )
            return {"node_properties": {}, "neighbors": []}
        ...
        for i, record in enumerate(instance):
            if not isinstance(record, dict):
                logger.warning(
                    f"Record at index {i} for NodeDetail is not a dict: {record}. Skipping."
                )
                continue

            rel_tuple = record.get("r")
            neighbor_node_dict = record.get(
                "m"
            )  # Assume 'm' is the neighbor relative to 'n'

            # Check if we have a valid relationship tuple and neighbor dict
            if (
                isinstance(rel_tuple, tuple)
                and len(rel_tuple) == 3
                and isinstance(neighbor_node_dict, dict)
            ):
                ...
                # If end node in tuple matches target node, then start node is the neighbor
                    ...
                    logger.warning(
                        f"Record {i}: Relationship tuple {rel_tuple} doesn't seem directly connected to target node {target_node_id}. Trying 'm' key."
                    )
                ...
                if current_neighbor_dict and current_neighbor_id:
                    ...
                    if current_neighbor_id not in neighbor_ids_seen:
                        neighbor_properties = {
                            k: str(v) for k, v in current_neighbor_dict.items()
                        }
                        ...
                        neighbors_data.append(
                            {
                                ...
                                "neighbor_labels": neighbor_properties.get(
                                    "labels", []
                                ),  # Get labels if key exists
                                "neighbor_name": neighbor_properties.get(
                                    "name", current_neighbor_id
                                ),
